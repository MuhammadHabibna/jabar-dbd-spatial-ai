...
# 2.   Feature Engineering (Time-Lag Effect)
# 3.  Advanced EDA
# 4.  Predictive Modeling (XGBoost)
# 5.   Spatial Modeling (Spatially Constrained Clustering)
...
# - Anomali El Nio 2023 (30% curah hujan, +0.5C suhu)
...
def generate_ir_dbd(row):
    ...
    - Suhu optimal 26-29C untuk nyamuk  +IR
    - Noise acak (realisme)
    """
    base      = 30.0
...
# %% [markdown]
# ##  CELL 3  Feature Engineering: Time-Lag Effect
#
...
ax1.set_title("Pola Musiman: Curah Hujan & Incidence Rate DBD\n"
              " Puncak DBD tertinggal 12 bulan dari puncak curah hujan (Time-Lag Effect)",
...
# %% [markdown]
# ##  CELL 6  Spatial Modeling: Spatially Constrained Clustering
#
...
ax_map.set_title(
    f"  Peta Klaster EWS-DBD  {last_date.strftime('%B %Y')}\n"
...
print("=" * 70)
print()
print(f"   Data     : 1.620 obs (27 kab/kota  60 bulan, Jan 2021Des 2025)")
print(f"    Features : {len(FEATURES)} fitur + 6 time-lag/rolling window")
print()
...
print(f"      R   : {r2:.4f}")
print()
print(f"    Spatial Clustering (Des 2025):")
...
print()
print("    Catatan Etika Algoritmik:")
